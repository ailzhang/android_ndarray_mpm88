...
public class Mpm88Runner {
    ...
    public Mpm88Runner(Context _context) {
        context = _context;
        // Open Json file.
        JSONParser parser = new JSONParser();
        InputStream jsonfile = null;
        if (USE_NDARRAY) {
            folder_name = "mpm88_ndarray_shaders/";
        } else {
            folder_name = "mpm88_field_shaders/";
        }
        try {
            jsonfile = this.context.getAssets().open(folder_name + "metadata.json");
        } catch(Exception e) {
            Log.d("Error", "Mpm88Runner: " + e);
        }
        JSONObject mpm88;
        try {
            ...
            Log.e("ERR", "Mpm88Runner: exception when parsing json: " + e);
            return;
        }
